...
public abstract class SendingNotification extends SendingAsync {
	final private static String TAG = "SendingNotification";
    ...
    protected void execute() {
        ...
        if (activeStreamServers.size() == 0) {
            Log.i(TAG , "Aborting notifications, no active stream servers found (network disabled?)");
            return;
        }
        ...
        for (int i = 0; i < getBulkRepeat(); i++) {
            try {
                ...
                Log.i(TAG , "Sleeping " + getBulkIntervalMilliseconds() + " milliseconds");
                Thread.sleep(getBulkIntervalMilliseconds());

            } catch (InterruptedException ex) {
                Log.i( TAG , "Advertisement thread was interrupted: " + ex);
            }
        }
    }
    ...
    public void sendMessages(Location descriptorLocation) {
        Log.i(TAG , "Sending root device messages: " + getDevice());
        ...
        if (getDevice().hasEmbeddedDevices()) {
            for (LocalDevice embeddedDevice : getDevice().findEmbeddedDevices()) {
                Log.i(TAG , "Sending embedded device messages: " + embeddedDevice);
        ...
        if (serviceTypeMsgs.size() > 0) {
            Log.i(TAG , "Sending service type messages");
    ...
    protected List<OutgoingNotificationRequest> createDeviceMessages(
    		LocalDevice device, Location descriptorLocation) {
