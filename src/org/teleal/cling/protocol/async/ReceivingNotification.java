...
public class ReceivingNotification extends ReceivingAsync<IncomingNotificationRequest> {
    final private static String TAG = "ReceivingNotification  >>> ";
...
//    	Utils.print(TAG , "execute");
        ...
        if (udn == null) {
            Utils.print(TAG , "Ignoring notification message without UDN: " + getInputMessage());
            return;
        }

        RemoteDeviceIdentity rdIdentity = new RemoteDeviceIdentity(getInputMessage());
//        Utils.print(TAG , "Received device notification: " + rdIdentity);
        ...
        if (getInputMessage().isAliveMessage()) {
            ...
            //Log.i(TAG , "Received device ALIVE advertisement, descriptor location is: " + rdIdentity.getDescriptorURL());
...
//            Utils.print(TAG, "go decriptors");
            ...
            //Log.i(TAG , "Received device BYEBYE advertisement");
            boolean removed = getUpnpService().getRegistry().removeDevice(rd);
            if (removed) {
                //Log.i(TAG , "Removed remote device from registry: " + rd);
            }

        } else {
            //Log.i(TAG , "Ignoring unknown notification message: " + getInputMessage());
        }
